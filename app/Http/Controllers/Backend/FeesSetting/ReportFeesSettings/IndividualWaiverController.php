...
class IndividualWaiverController extends Controller
{
    public function IndividualWaiverView(Request $request, $school_code)
    {
        ...
        return view('Backend.BasicInfo.FeesSetting.ReportFeesSettings.IndividualWaiver', compact('classes'));
    }
    ...
    public function GetStudentInformation(Request $request, $school_code)
    {
        ...
        foreach ($individualPaySlipsData as $key => $paySlip) {
            if (!in_array($paySlip->student_id, $studentInfo)) {
                $custom = $paySlip->student_id . ' - ' . $paySlip->name;
                $studentInfo[$paySlip->student_id] = $custom;
            }
    ...
    public function GetDataIndividualWaiver(Request $request, $school_code)
    {
        ...
        $individualWaiverData = Waiver::where("waivers.schoolCode", $school_code)
            ...
            ->when(isset($individualPaySlipsData[$student_id]), function ($query) use ($individualPaySlipsData, $student_id) {
                return $query->where('students.id', $individualPaySlipsData[$student_id]);
            })
