...
@section('Dashboard')
    ...
    <div class="gradient-bg relative mb-4 overflow-hidden rounded-md px-6 py-4 font-semibold text-white">
        <h2 class="">Add Admit Instruction</h2>
    ...
    <div class="relative overflow-x-auto rounded-md border-2 border-gray-300 bg-gray-200 p-6 sm:rounded-md">
        ...
        <div class="">
            <form
                class="space-y-4"
                action="{{ route('store.instructionInsert', $school_code) }}"
                method="POST"
                enctype="multipart/form-data"
            >
                @csrf
                <div>
                    <label for="Code" class="mb-2 block text-sm font-medium text-gray-900">Exam Instruction:</label>
                    <textarea
                        name="instruction"
                        id="message"
                        rows="6"
                        class="block w-full rounded-md border-0 border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 focus:border-blue-500 focus:ring-blue-500"
                        placeholder="Write maximum 125 Character"
                    ></textarea>
                    <h3 class="mt-1 text-xs text-rose-600">Please add maximum 4 instruction *</h3>
                </div>
                ...
                <div class="flex justify-end">
                    <button
                        type="submit"
                        class="rounded-md bg-blue-700 px-10 py-3.5 text-center text-sm font-medium text-white hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-300"
                    >
                        Saved
                    </button>
                </div>
