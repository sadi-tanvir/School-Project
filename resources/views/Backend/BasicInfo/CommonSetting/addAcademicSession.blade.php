...
<div class="relative overflow-x-auto shadow-md sm:rounded-lg mx-10 md:my-10">
    <div class="grid gap-6 mb-6 md:grid-cols-4 ">
        ...
        <div id="authentication-modal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
            <div class="relative p-4 w-full max-w-md max-h-full">
                <!-- Modal content -->
                <div class="relative bg-white rounded-lg shadow dd">
                    <!-- Modal header -->
                    ...
                    <div class="p-4 md:p-5">
                        <form action="{{ url('dashboard/addAcademicSession',$school_code) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div>
                                <label for="academic_session_name" class="block mb-2 text-sm font-medium text-gray-900 ">Academic Session :</label>
                                <input type="text" name="academic_session_name" id="academic_session_name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 " placeholder="enter session" required />
                            </div>
                            ...
                            <div class="hidden">
                                ...
                                <input type="text" value="{{$school_code}}" name="school_code" id="last_name"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dd  d"
        ...
        <input type="text" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dd  d" placeholder="" />
    </div>
    <table class="w-full text-sm text-left rtl:text-right text-black ">
        ...
        <tbody>
            @foreach($academicSessionData as $key=> $data)
