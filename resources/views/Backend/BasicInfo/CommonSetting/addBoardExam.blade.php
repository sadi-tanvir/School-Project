...
<div class="relative overflow-x-auto shadow-md sm:rounded-lg mx-10 md:my-10">
    <div class="grid gap-6 mb-6 md:grid-cols-4 ">
        <button data-modal-target="authentication-modal" data-modal-toggle="authentication-modal" type="button" class=" md:mr-20 text-white bg-blue-700 hover:bg-blue-600 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 ">Add
            New</button>
                ...
                <div class="relative bg-white rounded-lg shadow ">
                    ...
                    <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t ">
                        <h3 class="text-lg font-semibold text-gray-900 ">
                            NEW Board Exam Name ENTRY FORM
                        </h3>
                    ...
                    <div class="p-4 md:p-5">
                        <form action="{{ url('dashboard/addBoardExam',$school_code) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div>
                                <label for="board_exam_name" class="block mb-2 text-sm font-medium text-gray-900 ">Board Exam Name :</label>
                                <input type="text" name="board_exam_name" id="board_exam_name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5  " placeholder="enter board exam name" required />
                            </div>
                            ...
                            <div class="flex justify-center ">
                                <button type="submit" class=" mr-5 text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 ">Save</button>
    ...
    <table class="w-full text-sm text-left rtl:text-right text-black ">
        ...
        <tbody>
            @foreach($boardExamData as $key=> $data)
            <tr class=" border-b capitalize text-lg">
                <th scope="row" class="px-6 py-4 font-medium  text-black whitespace-nowrap ">
                    {{$key + 1}}
                </th>
                <td class="px-6 py-4">
                    {{$data->board_exam_name}}
                </td>
                <td class="px-6 py-4 ">
                    {{$data->position}}
                </td>
                ...
                <td class="px-6 py-4  text-xl flex justify-center">
                    ...
                    <form method="POST" action="{{ url('dashboard/delete_board_exam', $data->id) }}">
                        @csrf
                        @method('DELETE')
                        <button class="btn ">
