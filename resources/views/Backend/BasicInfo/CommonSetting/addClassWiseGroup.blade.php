    ...
    <form id="dataForm" method="POST" action="{{ route('add.class.wise.group',$school_code) }}">
        ...
        <div>
            <div class="grid gap-6 mb-6 py-5 md:grid-cols-3 items-center ps-4 border border-gray-200 rounded  mx-20">
                ...
                <div>
                    <input id="group_{{ $data->group_name }}" type="checkbox" value="{{ $data->group_name }}" name="group_name" class="group-checkbox w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 focus:ring-2  py-4 ms-2 text-sm font-medium text-gray-900 ">{{ $data->group_name }}</label>
                </div>
                @endforeach
            </div>
        </div>
