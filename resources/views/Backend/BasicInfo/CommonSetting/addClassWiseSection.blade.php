...
@section('Dashboard')
    ...
    <div class="gradient-bg relative mb-6 overflow-hidden rounded-md px-6 py-4 font-semibold text-white">
        <h2 class="">Class Setting/Section</h2>
    ...
    <div class="relative overflow-x-auto border-2 p-6 sm:rounded-md">
        <form id="dataForm" method="POST" action="{{ route('add.class.wise.section', $school_code) }}">
            @csrf
            <div class="">
                <div class="">
                    <label for="session" class="mb-2 block text-sm font-medium text-gray-900">Class Name :</label>
                </div>
                ...
                <div class="mt-4">
                    <div class="mb-6 grid items-center gap-6 rounded border border-gray-200 py-5 ps-4 md:grid-cols-3">
                        @foreach ($sectionData as $data)
                            <div>
                                <input
                                    id="section_{{ $data->section_name }}"
                                    type="checkbox"
                                    value="{{ $data->section_name }}"
                                    name="section_name"
                                    class="section-checkbox h-4 w-4 rounded border-gray-300 bg-gray-100 text-blue-600 focus:ring-2 focus:ring-blue-500"
                                />
                                <label
                                    for="section_{{ $data->section_name }}"
                ...
                <div class="text-end">
                    <button
                        type="button"
                        onclick="getData()"
                        class="w-full rounded-md bg-blue-700 px-5 py-2.5 text-center text-sm font-medium text-white hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-300 sm:w-auto"
                    >
                        GET DATA
                    </button>
                </div>
        ...
        <script>
            ...
            function getData() {
                ...
                var routeUrl = '{{ route('add.class.wise.section', $school_code) }}';
        ...
        <div class="mb-6 flex justify-center text-lg font-semibold">
            <h3>Class Wise Section Setting</h3>
        </div>
        <table class="w-full text-left text-sm text-black rtl:text-right">
            <thead class="bg-blue-600 text-xs text-white">
                <tr>
                    <th scope="col" class="bg-blue-500 px-6 py-3">SL</th>
                    <th scope="col" class="px-6 py-3">Class Name</th>
                    <th scope="col" class="bg-blue-500 px-6 py-3">Section Name</th>
        ...
        <div class="mr-32 mt-6 flex justify-end">
            <h3>Total = {{ $classWiseSectionData->count() }}</h3>
        </div>
    </div>
@endsection
