...
<div class="gradient-bg relative mb-6 overflow-hidden rounded-md px-6 py-4 font-semibold text-white">
    <h2 class="">Class By Subject Setup</h2>
...
<div class="relative overflow-x-auto sm:rounded-lg bg-gray-200 border-2 border-gray-300 p-6">
    ...
    <div class="flex justify-center text-lg font-semibold mb-3">
        <h3>
                ...
                @if ($classWiseSubjectData !== null)
                    @foreach ($classWiseSubjectData as $key => $data)
                        <tr id="row-{{ $data->id }}" class="border-b space-x-2 text-sm"
                            ...
                            <td class=" py-4">
                                <input type="number" name="subject_serial[{{ $data->id }}]"
                                    class="serial-input rounded-md w-full border-0 bg-white"
