...
<div class="relative overflow-x-auto shadow-md sm:rounded-lg mx-10 md:my-10">
    <div class="grid gap-6 mb-6 md:grid-cols-4 ">
        <a href="/dashboard/listSignature/{{$school_code}}"><button type="button" class=" md:mr-20 text-white bg-blue-700 hover:bg-blue-600 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2  focus:outline-none ">
                List
            </button></a>
    ...
    <div class="p-4 md:p-5">
        <form action="{{url('dashboard/AddSignature',$school_code)}}" method="POST" enctype="multipart/form-data">
            ...
            @method('PUT')
            <div>
                <label for="Code" class="block mb-2 text-sm font-medium text-gray-900 ">Signature Name :
                </label>
                <input type="text" name="sign" id="Code" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5    " placeholder="enter Exam Signature Name" required />
            </div>
            <div>
                <label for="position" class="block mb-2 text-sm font-medium text-gray-900 ">Signature :
                </label>
