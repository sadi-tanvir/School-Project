...
<form action="{{ url('dashboard/setShortCode', $school_code) }}" method="POST" enctype="multipart/form-data"
    class="relative overflow-x-auto border-2 bg-gray-200 p-6 shadow-md sm:rounded-lg">
    @csrf
    @method('PUT')
    <div>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
            <div>
                <div class="">
                    <label for="class_name" class="mb-2 block text-sm font-medium text-gray-900">Class Name:</label>
                </div>
                <select id="class_name" name="class_name" class="block w-full rounded-lg border-0 border-gray-300 bg-white p-3.5 text-sm text-gray-900 focus:border-blue-500 focus:ring-blue-500">
            ...
            <div>
                ...
                <select id="class_exam_name" name="class_exam_name"
                    class="block w-full rounded-lg border-0 border-gray-300 bg-white p-3.5 text-sm text-gray-900 focus:border-blue-500 focus:ring-blue-500">
            ...
            <div>
                <div class="">
                    <label for="academic_year_name" class="mb-2 block text-sm font-medium text-gray-900">
                        Year:
                    </label>
                </div>
                <select name="academic_year_name" id="date-academic_year_name"
                    ...
                    @elseif ($searchAcademicYearName)
                        <option value="{{ $searchAcademicYearName }}" selected>
                            {{ $searchAcademicYearName }}
                        </option>
                    @endif
            ...
            <div class="flex justify-end">
                ...
                <button type="button" onclick="submitForm()"
                    class="mb-2 me-2 rounded-lg bg-blue-700 px-10 py-3.5 text-sm font-medium text-white hover:bg-blue-600 focus:outline-none focus:ring-4 focus:ring-blue-300">
                    Get Data
                </button>
            </div>
        </div>
    ...
    <div class="text-md flex justify-center font-bold">
        <h2>CLASS WISE SHORT CODE SETTING</h2>
    </div>
    <table class="w-full text-left text-sm text-black rtl:text-right">
        <thead class="border-b border-blue-400 bg-blue-600 text-lg uppercase text-white">
            <tr>
                <th scope="col" class="px-6 py-3">SL</th>
                <th scope="col" class="bg-blue-500 px-6 py-3">SHORT CODE NAME</th>
        ...
        <tbody>
            @if ($shortCodeData !== null)
                    @foreach ($shortCodeData as $key => $data)
                            <tr class="border-b text-lg capitalize">
                                ...
                                <td class="px-6 py-4 text-center">
                                    ...
                                    @if ($setCode !== null)
                                        @foreach ($setCode as $code)
                                            @if ($data->short_code === $code->short_code)
                                                <input id="short_code" type="checkbox" value="{{ $code->short_code }}"
                                                    name="short_code[{{ $code->short_code }}]"
                                    ...
                                    @if (!$found)
                                        <input id="short_code" type="checkbox" value="{{ $data->short_code }}"
    ...
    <div class="grid md:grid-cols-3">
        <div class="mr-10 justify-center md:flex">
            <button type="submit"
        ...
        <div class="ml-32 justify-center md:flex">
            <h3>
                Total =
                @if ($shortCodeData !== null)
                    {{ $shortCodeData->count() }}
