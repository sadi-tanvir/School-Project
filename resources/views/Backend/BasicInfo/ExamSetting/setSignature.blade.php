...
@section('Dashboard')
    ...
    <div class="relative overflow-x-auto border-2 p-6 sm:rounded-lg">
        ...
        <div class="text-lg">
            <div class="text mb-5 flex justify-center font-bold">
                <h3>REPORT NAME WISE SIGNATURE SETTING</h3>
            </div>
            <form id="dataForm" method="POST" action="{{ route('store.signature', $school_code) }}">
                @csrf
                <table id="data" class="w-full text-left text-sm text-black rtl:text-right">
                        ...
                        @if (isset($selectedReport))
                            @foreach ([1, 2, 3] as $index)
                                <tr>
                                    ...
                                    />
                                    <td>
                                        {{ $index === 1 ? 'Class Teacher' : ($index === 2 ? 'Guardian' : 'Headmaster') }}
                                    </td>
                                    <td>
                                        <select
                                            name="positions[]"
                ...
                <div class="my-10 justify-center md:flex">
                    <div class="mr-10">
                        <button
                            type="submit"
