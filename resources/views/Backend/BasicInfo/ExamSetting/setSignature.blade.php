...
<div class="gradient-bg relative mb-6 overflow-hidden rounded-md px-6 py-4 font-semibold text-white">
    <h2 class="">Exam Report Name By Signature Setup</h2>
        ...
        <div class="md:flex my-10 ">
            ...
            <div class="mr-10">
                <select name="report_name"
                    class="block w-full rounded-lg border border-gray-300 bg-gray-50 p-3.5 text-sm text-gray-900 focus:border-blue-500 focus:ring-blue-500">
                    @if ($reportName !== null)
            ...
            <div>
                <button type="submit" id="getDataButton"
                    class="w-full rounded-lg bg-blue-700 px-10 py-3.5 text-center text-sm font-medium text-white hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-300 sm:w-auto">GET
                    DATA
                </button>
            </div>
                                ...
                                <td>
                                    <select name="positions[{{ $key }}]"
                                        ...
                                        @if($previouslySelectedReport && $previouslySelectedReport->status == 'active' && $previouslySelectedReport->signature_name === $signature->sign)
                                            <option value="{{$previouslySelectedReport->positions}}">
                                                {{$previouslySelectedReport->positions}}</option>
                                        @else
                                            <option disabled selected>Select</option>
                                        @endif
                                ...
                                <td>
                                    <div class="">
                                        <input id="laravel-checkbox-{{ $key }}" type="checkbox" value="active"
                                            name="status[{{ $key }}]" @if($previouslySelectedReport && $previouslySelectedReport->status == 'active' && $previouslySelectedReport->signature_name === $signature->sign) checked @endif
