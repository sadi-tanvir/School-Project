...
<form class="grid grid-cols-3 mx-5" action="{{route('get.subject.config.view', $school_code)}}"
    method="POST">
    @csrf
    <div class="flex gap-10">
        <label for="class" class="uppercase whitespace-nowrap">Select Class :</label>
        <select id="class" name="class"
            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-[60%] h-10  ">
    ...
    <div class="flex gap-10">
        <label for="group" class="uppercase whitespace-nowrap">Select Group :</label>
    ...
    <div class="">
        <button type="submit"
            class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-[30%] px-5 py-2.5 text-center  ">Search</button>
    </div>
</form>

<div class=" text-lg font-semibold mx-5">
    <div class="flex justify-center  mt-5 ">
        <h3 class="bg-blue-200 w-full text-center p-1">
            CLASS WISE SUBJECT REPORT Class : Group :
        </h3>
    </div>

    <table class="w-full ">
        <!-- text-xs text-center text-white uppercase bg-blue-600 border-b border-blue-400 -->
        <thead class="text-sm text-center text-white uppercase bg-blue-600 ">
            <tr>
        ...
        @if($subject != null)
            <tbody>
                @foreach ($subject as $data)
                    <tr class="text-xs  space-x-2 text-center table_body">
                        <td scope="col" class="px-6 py-2 text-left">
                            {{$data->subject_name}}
                        </td>
                        <td scope="col" class="px-6 py-2">
                            @if($data->subject_type != 'select')
                                {{$data->subject_type}}
                            @endif
                        </td>
                        <td scope="col" class="px-6 py-2">
