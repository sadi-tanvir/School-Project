...
<style>
    ...
    @include('Shared.ContentHeader', ['title' => 'Individual Payslip'])
...
<div class=" mt-10">
    ...
    <form action="{{ route('individualPaySlipReport.print', $school_code) }}"
        ...
        <div class="space-y-3">
            ...
            <div class="place-items-start  gap-5">
                <label for="pay_slip_type_name" class="block mb-2 text-sm font-medium whitespace-noWrap ">PAY SLIP :
                </label>
                <select id="pay_slip_type_name" name="pay_slip_type_name"
                    class="col-span-3 bg-white border-0  text-gray-900 text-sm rounded-lg  block w-full p-3.5">
                    <option disabled selected>Select </option>
                    @foreach ($paySlipTypes as $paySlipType)
                    <option value="{{ $paySlipType->pay_slip_type }}">
                        {{ $paySlipType->pay_slip_type }}
                    </option>
                    @endforeach
                </select>
            </div>
