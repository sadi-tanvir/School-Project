...
@include('Shared.ContentHeader', ['title' => 'Add Staff Designation'])
...
<div class="w-full rounded-md mx-auto p-6 bg-gray-200 border-2 border-gray-300 space-y-3">
    <div class="w-full flex justify-between items-center gap-4">
        <div class="flex space-x-6">
            <button data-modal-target="add_staff_designation_modal" data-modal-toggle="add_staff_designation_modal" type="button"
            ...
            {{-- Add Designation modal form start --}}
            <div id="add_staff_designation_modal" tabindex="-1" aria-hidden="true"
                    ...
                    <div class="relative bg-white rounded-lg shadow p-4">
                        <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t ">
                            <h3 class="text-xl font-semibold text-gray-900 ">
                                New Staff Designation entry form
                            </h3>
                            <button type="button"
                                class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center "
                                data-modal-hide="add_staff_designation_modal">
                        ...
                        <form method="POST" action="{{route("staffDesignation.create", $school_code)}}"
                            class="w-full mx-auto">
                            @csrf
                            <div class="p-4 md:p-5 space-y-4">

                                <div class="mb-5">
                                    <label for="designation" class="block mb-2 text-sm font-medium text-gray-900 ">Staff Designation:</label>
                            ...
                            <div class="flex items-center p-4 md:p-5 border-t border-gray-200 rounded-b ">
                                ...
                                <button data-modal-hide="add_staff_designation_modal" type="button"
            ...
            {{-- Add Designation modal form end --}}
        </div>

         {{-- search box --}}
        <form id="searchForm" class="mb-0 grow" method="GET" action="#">
        ...
        <div class="mt-10">
            <div class="relative overflow-x-auto sm:rounded-lg">
                <table class="w-full border text-sm text-left rtl:text-right text-gray-500 ">
                    ...
                    <tbody class="">
                        @foreach ($staffDesignation as $key => $designation)
                        <tr class="border-b">
                            ...
                            <td class="px-6 py-4 flex justify-center items-center space-x-5">
                                {{-- Delete Staff Designation --}}
                                <form method="POST"
                                    action="{{route("staffDesignation.delete", ["schoolCode" => $school_code, "id" => $designation->id])}}">
                                    @csrf
                                    @method('DELETE')
                                ...
                                <div class="mb-3">
                                    <svg data-modal-target="update_staff_designation_modal_{{ $designation->id }}"
                                        data-modal-toggle="update_staff_designation_modal_{{ $designation->id }}"
                                    ...
                                    {{-- Update Staff Designation modal form start --}}
                                    <div id="update_staff_designation_modal_{{ $designation->id }}" tabindex="-1" aria-hidden="true"
                                        class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                                        <div class="relative p-10 w-full max-w-2xl max-h-full">
                                            <div class="relative bg-white rounded-lg shadow ">
                                                <div
                                                    ...
                                                    <h3 class="text-xl font-semibold text-gray-900 ">
                                                        Update Staff Designation of <span class="text-blue-500 font-semibold">{{
                                                            $designation->designation }}</span>
                                                    </h3>
                                                    <button type="button"
                                                        class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center "
                                                        data-modal-hide="update_staff_designation_modal_{{ $designation->id }}">
                                                ...
                                                <form method="POST"
                                                    action="{{route("staffDesignation.update", ["schoolCode" => $school_code, "id" => $designation->id])}}"
                                                    class="">
                                                    @csrf
                                                    @method('PUT')
                                    ...
                                    {{-- Update Staff Designation modal form end --}}
                                </div>
                            </td>
                        </tr>
                        @endforeach
