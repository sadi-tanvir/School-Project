...
@include('/Message/message')
    ...
    <div class="relative overflow-x-auto rounded-md border-2 bg-gray-200 md:mb-10 md:p-5">
        <form action="{{ route('upload.excel') }}" method="post" enctype="multipart/form-data">
            @csrf

            <div class="mb-6 mt-2 grid gap-6 md:grid-cols-4">
            <div>
            ...
            <div>
                ...
                <div>
                    <label class="mb-2 block text-sm font-medium text-gray-900" for="user_avatar">Category</label>
                    <select
                        id=""
                        name="category"
                ...
                <div>
                    <label class="mb-2 block text-sm font-medium text-gray-900" for="user_avatar">Academic Year</label>
                    <select
                        id=""
                        name="year"
                        class="block w-full rounded-lg border-0 bg-white p-3.5 text-sm text-gray-900 focus:border-blue-500 focus:ring-blue-500"
                ...
                <div class="">
                    <label class="mb-2 block text-sm font-medium text-gray-900" for="user_avatar">Excel File</label>
                    <input
                        name="file"
                        class="block w-full cursor-pointer rounded-lg border border-gray-300 bg-gray-50 p-1 text-sm text-gray-900 focus:outline-none"
                ...
                <div>
                    <label class="mb-2 block text-sm font-medium text-gray-900" for="user_avatar">
                        Student ID with/without
                    </label>
                    <select
                        id=""
                        name="stu_id"
            ...
            <div class="items-center justify-end md:flex">
                <div class="">
                    <a
                        href="{{ route('download.demo', $school_code) }}"
                        class="me-2 flex items-center justify-center gap-3 rounded-lg bg-rose-600 px-5 py-3.5 text-sm font-medium text-white hover:bg-rose-600 focus:outline-none focus:ring-4 focus:ring-rose-600"
                    >
                        <span>Blank Excel Download</span>
                        <span>
                            <svg
                                width="20px"
                                height="20px"
                ...
                <div class="">
                    <button
                        type="submit"
                        class="rounded-lg bg-blue-700 px-7 py-3.5 text-sm font-medium text-white hover:bg-blue-600 focus:outline-none focus:ring-4 focus:ring-blue-300"
                    >
                        Submit
                    </button>
                </div>
...
<script>
    $(document).ready(function() {
        $('#class').change(function() {
            ...
            $.ajax({
                url: "{{ route('upload.get-groups', $school_code) }}",
                method: 'post',
                data: {
                    class: class_name,
                    _token: '{{ csrf_token() }}'
        ...
        $('#class').change(function() {
            var class_name = $(this).val();
            $.ajax({
                url: "{{ route('upload.get-sections', $school_code) }}",
                method: 'post',
                data: {
                    class: class_name,
                    _token: '{{ csrf_token() }}'
        ...
        $('#class').change(function() {
            var class_name = $(this).val();
            $.ajax({
                url: "{{ route('upload.get-shifts', $school_code) }}",
                method: 'post',
                data: {
                    class: class_name,
                    _token: '{{ csrf_token() }}'
