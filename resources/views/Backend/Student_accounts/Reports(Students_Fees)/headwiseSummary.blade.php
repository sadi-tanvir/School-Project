...
<div class=" mt-10">
    <form action="" class="p-5 shadowStyle rounded-[8px] border border-slate-300 w-2/5 mx-auto space-y-3">
            ...
            <div>
                ...
                <div class="relative max-w-sm">
                    <div class="absolute inset-y-0 start-0 flex items-center mt-3">
                        <input type="date" value="{{ date('Y-m-d') }}" name="date_from" id="date_from" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 p-1 block w-full" placeholder="" />
                    </div>
                </div>
            </div>
            <div>
                ...
                <div class="relative max-w-sm">
                    <div class="absolute inset-y-0 start-0 flex items-center mt-3">
                        <input type="date" value="{{ date('Y-m-d') }}" name="date_to" id="date_to" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 p-1 block w-full" placeholder="" />
                    </div>
                </div>
            </div>
            <div></div>
        ...
        {{-- student id --}}
        <div class="w-full grid grid-cols-4 place-items-start  gap-5">
        ...
        {{-- Head ID --}}
        <div class="grid grid-cols-4 place-items-start  gap-5">
            <label for="payslipType" class="block mb-2 text-sm font-medium whitespace-noWrap ">Head ID :
                :</label>
            <select id="payslipType" name="payslipType" class="bg-gray-50  text-gray-900 text-sm rounded-lg  block w-full p-2.5 col-span-3">
                <option selected>Select</option>
                @foreach ($payslipTypes as $payslipType)
                <option value="{{ $payslipType->pay_slip_type }}">{{ $payslipType->pay_slip_type }}</option>
                @endforeach
            </select>
        </div>
        {{-- Entry By --}}
        ...
        <div class="w-full flex justify-end">
            <button type="submit" class="w-1/3  text-white bg-blue-700 hover:bg-blue-600 focus:ring-0  font-medium rounded-lg text-sm px-3 py-2.5 me-2 mb-2 focus:outline-none">Print
            </button>
        </div>
    </form>
</div>
        ...
        classId.addEventListener('change', async (e) => {
            try {
                ...
                const res = await fetch(
                    `/dashboard/headwiseSummary/getStudentRoll/${schoolCode}?class_name=${className}`
                )
