...
@include('Shared.ContentHeader', ['title' => 'List Of Special Discount'])

<div class="mt-10">
    <form method="GET" action="{{ route('listOfSpecialDiscount.getData', $school_code) }}"
            ...
            async function getStudentInformation(class_name, group_name, section_name) {
                try {
                    const res = await fetch(
                        `/dashboard/listOfSpecialDiscount/getStudentInfo/${schoolCode}?class_name=${class_name}`
                    )
